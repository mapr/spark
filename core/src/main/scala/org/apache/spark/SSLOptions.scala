...
private[spark] object SSLOptions extends Logging {
  ...
  def parse(conf: SparkConf, ns: String, defaults: Option[SSLOptions] = None): SSLOptions = {
    ...
    val keyStorePassword = Option(hadoopConf.getPassword(s"$ns.keyStorePassword")) match {
      case Some(s) => Option(s.mkString)
      case None => conf.getWithSubstitution(s"$ns.keyStorePassword")
        .orElse(defaults.flatMap(_.keyStorePassword))
    }

    val keyPassword = Option(hadoopConf.getPassword(s"$ns.keyPassword")) match {
      case Some(s) => Option(s.mkString)
    ...
    val trustStorePassword = Option(hadoopConf.getPassword(s"$ns.trustStorePassword")) match {
      case Some(s) => Option(s.mkString)
