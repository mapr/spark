...
public class MultiauthWebUiFilter extends AuthenticationFilter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    boolean isLoggedOut = checkFroCookie(cookies, logoutCookieName);
    if (isLoggedOut) {
      cleanCookieForResponse(cookies, httpResponse);
      httpResponse.sendRedirect("/login");
      return;
    }
    boolean isAuthorized = checkFroCookie(cookies, signedInCookieName);
  ...
  private boolean checkFroCookie(Cookie[] cookies, String cookieName) {
