...
class OracleIntegrationSuite
  ...
  override def dataPreparation(conn: Connection): Unit = {
    conn
      .prepareStatement(
        "CREATE TABLE datetime (id NUMBER(10), d DATE, t TIMESTAMP)")
      .executeUpdate()
    ...
    conn
      .prepareStatement(
        "CREATE TABLE datetime1 (id NUMBER(10), d DATE, t TIMESTAMP)")
      .executeUpdate()
    conn.commit()
    ...
    conn
      ...
      .executeUpdate();
    conn
      .prepareStatement("INSERT INTO numerics VALUES (4, 1.23, 9999999999)")
      .executeUpdate();
    conn.commit();
  }
  ...
  test("SPARK-16625: General data types to be mapped to Oracle") {
    ...
    assert(
      values
        .getDecimal(0)
        .compareTo(BigDecimal.valueOf(if (booleanVal) 1 else 0)) == 0)
  ...
  test("SPARK-19318: connection property keys should be case-sensitive") {
    def checkRow(row: Row): Unit = {
      ...
      assert(
        row.getTimestamp(2).equals(Timestamp.valueOf("1996-01-01 01:23:45")))
    }
