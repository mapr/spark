...
class AllTests extends FlatSpec with InitClusterDisableKryoSerialization with GivenWhenThen with Matchers {
...
//  }
  ...
  "saving a table" should "be tested" in {
    assert(com.mapr.db.testCases.LoadAndSaveTests.testingSimpleSaveTable(sc, tableName, tableName2+"new"))
  }

  "saving a table with different ID" should "be tested" in  {
    assert(com.mapr.db.testCases.LoadAndSaveTests.testingIDwithSaveToMapRDB(sc, tableName, tableName2+"new"))
  }

  "saving a table using bulk save mode" should "be tested" in {
    assert(com.mapr.db.testCases.LoadAndSaveTests.testingBulkSaveMode(sc, tableName, tableName2+"new1"))
  }
  ...
  "saving a table using bulk save mode with bean class" should "be tested" in {
    assert(com.mapr.db.testCases.LoadAndSaveTests.testingBulkSaveModeBeanClass(sc, tableName, tableName2+"new2"))
  }
...
//  }
  ...
  "testingTimeStampVsArray" should "be tested" in{
