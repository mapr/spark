...
object MapRSqlUtils {
  ...
  def documentsToRow(documents: Iterator[DocumentReader], schema: StructType,
                     requiredColumns: Array[String] = Array.empty[String]): Iterator[Row] = {
    documents.flatMap(record => convertRootField(record, schema) match {
  ...
  def convertRootField(docReader: DocumentReader,
                       schema: DataType): Any = {
  ...
  private def convertArray(documentReader: DocumentReader,
    ...
    while (event != null && event.compareTo(DocumentReader.EventType.END_ARRAY) != 0) {
      values += convertField(event, documentReader, array.elementType, fieldName)
      event = documentReader.next
    }
    values.toArray
  }
  ...
  private def convertMap(documentReader: DocumentReader,
    ...
    while (event!= null && event.compareTo(DocumentReader.EventType.END_MAP) != 0) {
      ...
      values += convertField(event, documentReader, mapType.valueType, documentReader.getFieldName)
      event = documentReader.next
    }
  ...
  def convertField(event: DocumentReader.EventType,
                   ...
                   schema: DataType, fieldName: String): Any = {
    ((event, schema)) match {
      ...
      case (DocumentReader.EventType.START_MAP, mt @ MapType(StringType, kt, _)) => {
      ...
      case (DocumentReader.EventType.NULL | null, _) => {
        null
      }

      case (DocumentReader.EventType.STRING, _) if documentReader.getString.length < 1 => null
      ...
      case (DocumentReader.EventType.STRING, DateType) => {
        new java.sql.Date(ODate.parse(documentReader.getString).toDaysSinceEpoch)
      }

      case (DocumentReader.EventType.STRING, TimestampType) => {
        new java.sql.Timestamp(OTimestamp.parse(documentReader.getString).getMillis)
      }
      ...
      case (token, dataType) => {
        if (isInvalidType(dataType)) throw new SchemaMappingException(s"Schema cannot be inferred for the column {$fieldName}")
        throw new SchemaMappingException(s"Failed to parse a value for data type $dataType (current token: $token)")
      }
    }
  }

  def isInvalidType(dt: DataType) : Boolean = {
    if(dt.isInstanceOf[StructType]) {
      val fields = dt.asInstanceOf[StructType].fields
      if (fields.size == 1 && fields.map(_.name).contains("InvalidType")) return true
    }
    return false
  }
  ...
  def rowToDocument(row: Row): OJAIDocument = {
    ...
    row.schema.fields.zipWithIndex.foreach({
      case (field, i) if !row.isNullAt(i) => document.set(field.name, convertToDataType(row.get(i), field.dataType))
  ...
  def convertToDataType(element: Any, elementType: DataType): org.ojai.Value = {
    (elementType) match {
      ...
      case (arrayType: ArrayType) => {
        ...
        DBValueBuilderImpl.KeyValueBuilder.initFrom(seq.map(elem => convertToDataType(elem, arrayType.elementType)).asJava)
      }
      case (mpType: MapType) => {
      ...
      case (LongType) => DBValueBuilderImpl.KeyValueBuilder.initFrom(element.asInstanceOf[Long])
      case (StringType) => DBValueBuilderImpl.KeyValueBuilder.initFrom(element.asInstanceOf[String])
      case (CalendarIntervalType) => DBValueBuilderImpl.KeyValueBuilder.initFrom(element.asInstanceOf[OInterval])
      case (DecimalType()) => DBValueBuilderImpl.KeyValueBuilder.initFrom(element.asInstanceOf[java.math.BigDecimal])
      case (FloatType) => DBValueBuilderImpl.KeyValueBuilder.initFrom(element.asInstanceOf[Float])
      case (ShortType) => DBValueBuilderImpl.KeyValueBuilder.initFrom(element.asInstanceOf[Short])
      case (ByteType) => DBValueBuilderImpl.KeyValueBuilder.initFrom(element.asInstanceOf[Byte])
      case _ => throw new RuntimeException(s"Cannot convert ${elementType} of a row to OjaiDocument's type")
    }
  }
}
