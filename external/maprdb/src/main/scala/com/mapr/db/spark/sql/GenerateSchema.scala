...
object GenerateSchema {
  ...
  private def DecimalTypeforType(dataType: DataType): DecimalType = dataType match {
  ...
  def apply(sc: SparkContext, tableName: String, sampleSize: Double): StructType = {
    apply(MapRSpark.load(sc, tableName), sampleSize)
  }

  def apply(rdd: MapRDBBaseRDD[OJAIDocument], sampleSize: Double) : StructType = {
    apply(rdd,sampleSize, false)
  }

  def apply(rdd: MapRDBBaseRDD[OJAIDocument], sampleSize: Double, failureOnConflict: Boolean): StructType = {

    val numOfPartitions = rdd.getNumPartitions
    val eachPartitionLimit = java.lang.Math.ceil(sampleSize/ numOfPartitions).toInt
    val sampleData: RDD[DocumentReader] = rdd.mapPartitions(iter => iter.take(eachPartitionLimit)).map(doc => doc.asReader())

    val resultType = sampleData.map(reader => inferTypeForField(failureOnConflict)(reader.next, reader))
                                 .treeAggregate[DataType](StructType(Seq()))(compatibleType(failureOnConflict), compatibleType(failureOnConflict))
  ...
  private def inferTypeForField(failureOnConflict: Boolean)(event: DocumentReader.EventType, reader: DocumentReader): DataType = {
    (event) match {
      ...
      case DocumentReader.EventType.START_ARRAY => {
        ...
        while (thisEvent != DocumentReader.EventType.END_ARRAY) {
          elementType = compatibleType(failureOnConflict)(elementType, inferTypeForField(failureOnConflict)(thisEvent, reader))
      ...
      case DocumentReader.EventType.START_MAP => {
        ...
        while (thisEvent != DocumentReader.EventType.END_MAP) {
          builder += StructField(reader.getFieldName, inferTypeForField(failureOnConflict)(thisEvent, reader), nullable = true)
          thisEvent = reader.next
        }
      ...
      case DocumentReader.EventType.DECIMAL => DecimalType(reader.getDecimalPrecision, reader.getDecimalScale)
      ...
      case _ => throw new RuntimeException(s"Type ${Option(event).toString} cannot be inferred")
    }
  }

  private def compatibleType(failureOnConflict: Boolean)(dt1: DataType, dt2: DataType): DataType = {
      ...
      ((dt1, dt2)) match {
        case (st1 @ StructType(fields1), st2 @ StructType(fields2)) => {
          ...
          val newFields = (fields1 ++ fields2).groupBy(field => field.name).map {
            case (name, fieldTypes) => try {
              val dataType = fieldTypes.view.map(_.dataType).reduce(compatibleType(failureOnConflict))
              StructField(name, dataType, nullable = true)
            } catch {
              case e: SchemaMappingException => throw new SchemaMappingException(s"Schema cannot be inferred for the column ${name}")
            }
          }
        ...
        case (ArrayType(elementType1, containsNull1), ArrayType(elementType2, containsNull2)) => {
          ArrayType(compatibleType(failureOnConflict)(elementType1, elementType2), containsNull1 || containsNull2)
        }
        ...
        case (t1: DataType, t2: DecimalType) if(isIntegral(t1)) => compatibleType(failureOnConflict)(DecimalTypeforType(t1), t2)

        case (t1: DecimalType, t2: DataType) if(isIntegral(t2)) => compatibleType(failureOnConflict)(t1, DecimalTypeforType(t2))
        ...
        case (_, _) => { if (failureOnConflict) throw new SchemaMappingException(s"Schema cannot be inferred")
  ...
  def isIntegral(dt: DataType): Boolean = {
    if (dt == LongType ||
        ...
        dt == ShortType ||
        dt == FloatType ||
        dt == DoubleType ) return true
    return false
  }

  def isDateOrTime(dt: DataType): Boolean = {
    if (dt == DateType ||
        dt == TimestampType) return true
    else return false
  }

  def isInvalidType(st: StructType): Boolean = {
    if (st.fieldNames.contains("InvalidType")) return true
    else return false
  }
}
