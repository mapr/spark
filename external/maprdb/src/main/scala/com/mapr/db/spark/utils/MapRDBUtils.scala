...
private[spark] object MapRDBUtils {
  def checkOrCreateTable(tableName: String, bulkMode: Boolean, createTable: Boolean, keys : Seq[org.ojai.Value]): (Boolean, Boolean) = {
    var isBulkLoad: Boolean = bulkMode
    if (!DBClient().tableExists(tableName)) {
      if (!createTable) throw new TableNotFoundException("Table: "+ tableName + " not found")
      ...
      return (true,bulkMode)
    } else if (createTable ) {
      throw new TableExistsException("Table: " + tableName + " already Exists")
    }

    if (bulkMode == true) {
      isBulkLoad = DBClient().isBulkLoad(tableName)
    }
    return (false, isBulkLoad)
  }
  ...
  def containsRow(row : Array[Byte], rowkeyRange: RowkeyRange): Boolean = {
    return Bytes.compareTo(row, rowkeyRange.getStartRow) >= 0 &&
  ...
  def readBytes(buff: ByteBuffer, bufferSize: Int, objectInput : ObjectInput): Unit = {
  ...
  def convertToMap(value : Map[String,Any]): Map[String, AnyRef] = {
    if (value.isInstanceOf[DBMapValue]) {
      value.asInstanceOf[DBMapValue].value.map{ case (k,v) => k -> v.asInstanceOf[AnyRef]}
    } else {
  ...
  def convertToScalaCollection(value: Any): AnyRef = {
    if (value.isInstanceOf[DBMapValue]) {
      ...
      return value.asInstanceOf[Map[String,Any]].map{case (k,v) => k -> convertToScalaCollection(v)}.asJava.asInstanceOf[AnyRef]
    } else if (value.isInstanceOf[Seq[Any]]) {
      return value.asInstanceOf[Seq[Any]].map(convertToScalaCollection(_)).asJava.asInstanceOf[AnyRef]
    } else {
  ...
  def getOjaiKey[T : ClassTag]() : OJAIKey[T] = {
    ...
    val result = classTag[T] match {
      case a if a <<: classTag[String] => return OJAIKey.ojaiStringKey.asInstanceOf[OJAIKey[T]]
      case a if a <<: classTag[ByteBuffer] => return OJAIKey.ojaibytebufferKey.asInstanceOf[OJAIKey[T]]
      case a if a <<: classTag[DBBinaryValue] => return OJAIKey.ojaibinaryKey.asInstanceOf[OJAIKey[T]]
      case _ => throw new RuntimeException("Key with type:" + classTag[T].runtimeClass + " is not supported")
    }
